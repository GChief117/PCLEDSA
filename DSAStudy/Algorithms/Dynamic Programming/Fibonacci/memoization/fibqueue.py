...
# Insert directories into a queue while calculating Fibonacci for each path
def insert_in_queue_fib(root_directory):
    queue = Queue()
    memo = {}
    ...
    while queue.queue:
        fibonacci_memo(len(queue.queue.popleft()), memo)
    end_time = time.time()
...
insert_in_queue_fib(root_directory)
