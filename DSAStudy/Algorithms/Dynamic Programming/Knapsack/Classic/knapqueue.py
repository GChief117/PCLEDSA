...
class Queue:
    ...
    def enqueue(self, data, weight, value):
        self.queue.append((data, weight, value))
...
def insert_in_queue(root_directory):
    queue = Queue()
    total_capacity = 0
    ...
    for dirpath, dirnames, filenames in os.walk(root_directory):
        for filename in filenames:
            ...
            try:
                ...
                queue.enqueue(file_path, file_size, file_size)  # Enqueue into queue
    ...
    return queue, total_capacity

# Main function to run knapsack with queue and dynamic capacity
def run_knapsack_with_dynamic_queue(root_directory):
    # Traverse the directory and insert data into the queue
    queue, total_capacity = insert_in_queue(root_directory)

    # Retrieve weights and values from the queue
    weights, values, dynamic_capacity = queue.get_weights_and_values()
...
# Example usage for knapsack with queue
if __name__ == "__main__":
    root_directory = "/Volumes/CD/pCLE"  # Replace with your actual root directory
    run_knapsack_with_dynamic_queue(root_directory)
